...
class TradeHistoryStorage(context: Context) {
    ...
    fun saveAll(list: List<TradeHistoryItem>) { prefs.edit().putString("trades", gson.toJson(list)).apply() }
    fun saveTrade(item: TradeHistoryItem) { val list = getAllTrades().toMutableList(); list.add(0,item); saveAll(list) }
    fun saveSimulatedTrade(side: String, entry: Double, sl: Double, tp: Double, live: Double) {
        saveTrade(TradeHistoryItem(orderId="SIM_${System.currentTimeMillis()}", side=side, entryPrice=entry, sl=sl, tp=tp, currentPrice=live, status="LIVE", mode="FREE"))
    }
    ...
    fun checkIfNigo(entry: Double, sl: Double, tp: Double, side: String, livePrice: Double): Pair<Boolean?, String> {
        val isBuy = side=="BUY"
        return when {
            ...
            !isBuy && livePrice >= sl -> Pair(false, "X PILDI SL")
                ...
                val pct = try { ((livePrice-entry)/(tp-entry)*100).toInt() } catch(e:Exception){0}
    ...
    fun updateLiveTrade(orderId: String, livePrice: Double, stage: String) {
        ...
        if(idx!=-1) {
            ...
            val newStatus = when(nigo){ true->"WIN_✓"; false->"LOSS_X"; null->"LIVE" }
            list[idx]=old.copy(currentPrice=livePrice, isNigo=nigo, status=newStatus, tpSmartStage=stage)
            saveAll(list)
        }
    }
    fun getStats(): String {
    ...
    fun getDailyLoss(): Double = getAllTrades().filter{it.timestamp > System.currentTimeMillis()-86400000 && it.status.contains("LOSS")}.sumOf{ abs(it.pnl) }
}
